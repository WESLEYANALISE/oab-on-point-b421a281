...
function NoArvore({ no, nivel, onOpen }: { no: Nó; nivel: number; onOpen: (id: string) => void }) {
  ...
  return (
    <li className={nivel === 0 ? "rounded-xl border border-border/60 bg-card/40 overflow-hidden" : "border-l border-border/40 ml-2 pl-2 mt-1"}>
      <button
        type="button"
        onClick={() => setAberto(!aberto)}
        className="w-full flex items-start gap-3 px-3 py-2.5 text-left hover:bg-card/60 rounded-lg transition-colors"
      >
        <span className="shrink-0 mt-0.5 h-7 px-2 rounded-md bg-gradient-to-br from-gold/20 to-amber-700/10 border border-gold/30 grid place-items-center text-[10px] font-semibold uppercase tracking-wider text-gold whitespace-nowrap">
          {no.rotulo}
        </span>
        ...
        <ChevronRight className={`h-4 w-4 text-muted-foreground shrink-0 mt-1 transition-transform ${aberto ? "rotate-90" : ""}`} />
      </button>
      {aberto && (
        <div className="px-2 pb-2">
          {no.filhos.length > 0 && (
            <ul>{no.filhos.map((f) => <NoArvore key={f.id} no={f} nivel={nivel + 1} onOpen={onOpen} />)}</ul>
          )}
          {no.artigos.length > 0 && (
